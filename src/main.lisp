...
(defun chimelisp/compile ()
  (let* ((context "")
	 ...
	 (abstraction-1 (make-instance 'abstraction :context "" :body literal :arguments '("x" "y") :name "xyz" :return-type "i32"))
	 (application (make-instance 'application :context ""
						  :func abstraction-1
						  :arguments (list (make-instance 'literal :value 1 :context "")
								   (make-instance 'literal :value 2 :context ""))))
	 (abstraction
    ...
    (with-open-file (stream file-name :direction :output)
      (format stream "(module (import \"env\" \"print_string\" (func $print_string (param i32))) (import \"env\" \"buffer\" (memory 1)) ~a ~a)" (context abstraction-1) compiled-context))
